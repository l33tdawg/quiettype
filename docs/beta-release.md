...
```text
dist/QuietType-1.0.3-macOS-arm64.dmg
dist/QuietType-1.0.3-macOS-arm64.dmg.sha256
```
...
```bash
bash scripts/validate-release-artifact.sh dist/QuietType-1.0.3-macOS-arm64.dmg
```
...
```bash
QUIETTYPE_VALIDATE_LAUNCH=1 bash scripts/validate-release-artifact.sh dist/QuietType-1.0.3-macOS-arm64.dmg
```
...
```bash
VERSION="v1.0.3"
DMG="dist/QuietType-1.0.3-macOS-arm64.dmg"

git tag -a "$VERSION" -m "QuietType 1.0.3"
git push origin "$VERSION"

gh release create "$VERSION" "$DMG" "$DMG.sha256" \
  --repo l33tdawg/quiettype \
  --verify-tag \
  --title "QuietType 1.0.3" \
  --notes "QuietType 1.0.3 for macOS Apple Silicon. Local dictation, local memory, no cloud processing."
```
...
tag like `v1.0.3`, it creates a public GitHub release; prerelease tags remain supported.
